...
|---|---|---|
| [`APRENDIZADOS-SEO-IA-20260910.md`](APRENDIZADOS-SEO-IA-20260910.md) | Integração de 63 conceitos, survey e evidências recentes; limites de seleção/absorção, reconciliação da KB/OS e correção do plano do Paper 2. | Antes de interpretar proxies como métricas semânticas, desenhar novos estudos ou reutilizar recomendações de ondas anteriores. |
...
| [`HEALTH-CHECK-APIS-20260908.md`](HEALTH-CHECK-APIS-20260908.md) | Dezessete dias de série perdidos por saldo esgotado (16-08 a 08-09), `MANDATORY_LLMS` rebaixado, clone local com banco velho, CI vermelho sem defeito e a retirada da Sonar Chat Completions em 27-09 com o gate `PAPERS_PPLX_AGENT_API`. | Antes de citar progresso da janela (fazer `r2_sync.py pull` primeiro); ao investigar falha de preflight por crédito; antes de 27-09-2026 para decidir a virada da Agent API. |
| [`CHAVES-COMPARTILHADAS-ENTRE-INSTRUMENTOS-20260909.md`](CHAVES-COMPARTILHADAS-ENTRE-INSTRUMENTOS-20260909.md) | As mesmas chaves de API servem `papers`, `geo-orchestrator` e o monitor de citação do `landing-page-geo`; uma conta esgotada derrubou os três entre 28-08 e 08-09. Recomendação: chave por instrumento, teto por projeto, alerta em dias de coleta. | Antes de criar ou rotacionar chave; ao abrir incidente de saldo, para não duplicar em três repositórios. |
