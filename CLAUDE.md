...
**Wave Setembro 2026 (03-09-2026)** — consolida maio a setembro e em parte **CORRIGE** a Wave Agosto; cinco provedores com web ao vivo (Gemini 3.1 Pro grounding ×2, xAI `grok-4.6` Agent Tools API ×1, OpenAI `gpt-5.5` web_search ×2 concluídas e 6 por ReadTimeout, Perplexity **web** ×7 pelo navegador logado porque a API devolveu `401 insufficient_quota`) + doublecheck Claude (listagem de busca do arXiv, 24 IDs abertos na página `abs`, 49 URLs com sentinela):
- [`docs/research/geo-wave-setembro-2026/GEO_WAVE_SETEMBRO_2026_CANONICAL.md`](docs/research/geo-wave-setembro-2026/GEO_WAVE_SETEMBRO_2026_CANONICAL.md) — **doc canônico** delta 27-ago → 03-set sobre corpus maio a setembro (56 arXiv IDs). Núcleo: (a) **medição oficial chegou e é só impressão**: relatório de IA generativa do Search Console e controle de exclusão para todos os sites em 31-ago (impressões em AIO e AI Mode por página, país, dispositivo, data; sem cliques); (b) **AI Mode troca de modelo dentro do ciclo de report**: Gemini 3.7 Flash em agosto, 3.8 Flash em 02-set, relatos de respostas sem link em 03-set → toda leitura de AI Mode declara modelo e data; (c) **ChatGPT Ads US$ 1 bi anualizado em 31-ago** (OpenAI; 40+ países; CPC majoritário); 26% das respostas no desktop dos EUA com anúncio em junho, CTR 0,50% (Similarweb 03-set); primeiros anúncios no Brasil em 04-ago (Meio & Mensagem), lançamento oficial 11-ago; (d) **feed antes de página no comércio**: recuperação por feed integrado no Shopping do ChatGPT 8,26% → 61,54% após o GPT 5.6 (Profound 03-set); (e) **Reddit 3,8% → 0,5%** nas citações do ChatGPT (Semrush/Promptwatch 26-ago); busca 84 threads e cita zero (Suganthan): insumo invisível; (f) **marca recomendada ≠ marca citada**: página própria é 2,8% das fontes citadas em 60 categorias de compra (Shero); UGC ≈ 50% (Trendos); AIO nos EUA: YouTube 22,9%, Reddit 18,5% (Ahrefs 02-set); (g) **corpus**: Counter-GEO-Bench `2609.02316` (guardrails de prateleira reduzem ataque ≤5,7%; detector leve 47,6%), CHASE `2608.30466` (otimizar contra o mesmo ranking degrada qualidade em 6 domínios), VCR `2608.11390` (recompensa por conteúdo verificável +12,1 pontos), regra de parada por convergência `2607.10341` (substitui N fixo), experimento causal `2608.18352` (remover AIO/AI Mode aumenta cliques), painel Pew `2608.04831` (clique em fonte de AIO ≈1%), `2609.00319` (10 domínios = 43,6% das citações; penalidade de idioma), PersonaGen-1M `2608.30023`, Agent2UCB `2608.29063`, competitor-aware `2608.27631`, `2607.04282` v3 (assistente fica no meio da jornada, não na boca); (h) **framework de execução v2** (§4: opt-in registrado, GSC como série oficial, universo de prompts pela demanda, parada por convergência, fontes citáveis por motor, feeds e agent readiness em 3 camadas, evidência verificável, dono do feed) e **report em 3 níveis** reconciliado com Semrush 02-set (§5); (i) prazos: Cloudflare 15-set, dataset europeu do Google 17-set, **Sonar da Perplexity aposentado 27-set**, ChatGPT designado VLOSE na UE 31-ago; Common Crawl: 584.107 `llms.txt`, 68% por plugin, 22,56% sem link; Mueller: só ferramentas de SEO pedem Markdown. **§8 CORRIGE**: spam update terminou em 21-ago (não 20); AI Mode exige modelo declarado; N fixo sai, regra de parada entra; Reddit vira insumo invisível; campo `search_queries` do ChatGPT desapareceu (medir pelo slot de domínio); `sonar-pro` deixa de ser plano B em 27-set; feed antes de página; GEO = camada sobre SEO é consenso público. Raw em [`raw/`](docs/research/geo-wave-setembro-2026/raw/). **§9.2 = aplicação neste repo** (ingestão tagueada de 20 IDs; regra de parada `2607.10341` na coleta e modelo ativo por braço; `2609.00319` como comparável da hipótese pt-BR; braço Perplexity migra para a Agent API antes de 27-set e sem crédito fica `api_failure`; extrator novo para o slot de domínio do ChatGPT; `2608.18352`, `2608.04831`, `2607.04282` no related work).
